...
namespace KPLN_BIMTools_Ribbon.ExternalCommands
{
    ...
    internal class CommandIFCExport : ExchangeService, IExternalCommand, IExecuteByUIApp
    {
        internal const string PluginName = "IFC: Экспорт";

        public CommandIFCExport()
        {
        }
        ...
        {
            return ExecuteByUIApp(commandData.Application, RevitDocExchangeEnum.IFC);
        }
        ...
        {
            //Апкастинг в настройку для экспорта в IFC
            if (!(configEntity is DBIFCConfigData ifcConfigData))
                throw new Exception($"Скинь разработчику: Не удалось совершить корректный апкастинг из {nameof(DBConfigEntity)} в {nameof(DBIFCConfigData)}");


            // Проверка на Revit Server
            if (!string.IsNullOrWhiteSpace(rsn))
            {
                Module.CurrentLogger.Error("Экспорт IFC на Revit Server не поддерживается. Нужно указать обычную сетевую/локальную папку.");
                return null;
            }
            ...
            {
                if (!WSName_IsMatchByRules(worksetPrev.Name, ifcConfigData.WorksetToCloseNamesStartWith))
                {
            ...
            try
            {
                ...
                IEnumerable<View3D> currentDoc3DViews = new FilteredElementCollector(doc)
                    ...
                    .Where(e => e.Name.Equals(ifcConfigData.ViewName))
                    .Select(e => e as View3D);
                if (!currentDoc3DViews.Any())
                {
                    Module.CurrentLogger.Error($"Не удалось найти вид с именем ({ifcConfigData.ViewName}). Либо конфигурация не верная, либо такого вида в проекте нет. Нужно вмешаться человеку");
                    return null;
                }
                ...
                {
                    Module.CurrentLogger.Error($"На виде с именем ({ifcConfigData.ViewName}) НЕТ элементов для экспорта. Нужно вмешаться человеку");
                    return null;
                }
                #endregion


                #region Устанавливаем параметры экспорта в IFC
                IFCExportOptions exportOptions = new IFCExportOptions
                {
                    FileVersion = ifcConfigData.FileVersion,
                    SpaceBoundaryLevel = ifcConfigData.SpaceBoundaryLevel,
                    WallAndColumnSplitting = ifcConfigData.WallAndColumnSplitting,
                    ExportBaseQuantities = ifcConfigData.ExportBaseQuantities,
                    FilterViewId = viewId
                };

                // Доп. опции IFC-экспортера.
                // Если конкретная версия экспортера опцию не поддерживает, она будет проигнорирована.
                exportOptions.AddOption("ExportLinkedFiles", ifcConfigData.ExportLinks.ToString());
                exportOptions.AddOption("VisibleElementsOfCurrentView", "true");
                #endregion


                #region Экспорт в IFC
                string folderTo = ifcConfigData.PathTo;
                ...
                CurrentDocName = $"{docTitle}{ifcConfigData.IfcDocPostfix}.ifc";

                using (Transaction transaction = new Transaction(doc, "IFC Export"))
                {
                    transaction.Start();
            ...
            catch (Exception ex)
            {
                Module.CurrentLogger.Error(
                    $"Не удалось экспортировать IFC ({ModelPathUtils.ConvertModelPathToUserVisiblePath(modelPathFrom)}). " +
                    $"Ошибка: {ex.Message}");
            ...
            finally
            {
                if (doc != null && doc.IsValidObject)
                {
                    ...
                    {
                        Module.CurrentLogger.Error($"Не удалось закрыть документ после IFC-экспорта. Ошибка: {ex.Message}");
                    }
                }
            }            
        }
