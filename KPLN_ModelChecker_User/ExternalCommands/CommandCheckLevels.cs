...
namespace KPLN_ModelChecker_User.ExternalCommands
{
    ...
    internal class CommandCheckLevels : AbstrCheckCommand<CommandCheckLevels>, IExternalCommand
    {
        public CommandCheckLevels() : base()
        {
        }

        internal CommandCheckLevels(ExtensibleStorageEntity esEntity) : base(esEntity)
        {
        }
        ...
        public override Result ExecuteByUIApp(UIApplication uiapp)
        {
            ...
            Element[] levels = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Levels).WhereElementIsNotElementType().ToArray();
            #region Проверяю и обрабатываю элементы
            WPFEntity[] wpfColl = CheckCommandRunner(doc, levels);
