...
namespace KPLN_CoordiantorAI.ExternalAIModel.Mcp
{
    internal static class RevitMcpToolRegistry
    {
        private static readonly List<RevitMcpToolDefinition> Tools = new List<RevitMcpToolDefinition>
        {
            ...
            Tool("get_phase_visibility_settings", "Visibility", "Returns the view phase, discipline, assigned phase filter settings and phase status for supplied elements. Visibility conclusions cover phase rules only. Element results are paginated.", PagedProps(new[] { new JProperty("viewId", Int("View id. Optional; defaults to the active view.")), new JProperty("list_elementIds", IntArray("Optional element ids whose phase status should be evaluated.")), new JProperty("includeAllFilters", Bool("Include every phase filter defined in the model. Default false.")) })),
