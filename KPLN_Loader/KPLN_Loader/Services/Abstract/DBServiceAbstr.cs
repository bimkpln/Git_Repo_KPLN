...
namespace KPLN_Loader.Services.Abstract
{
    /// <summary>
    /// Сервис для работы с БД
    /// </summary>
    internal abstract class DBServiceAbstr<TConnection, TDbException>
        ...
        where TDbException : Exception
    {
        ...
        public DBServiceAbstr(Logger logger, string connectionStringOrPath)
        {
            _logger = logger;
        ...
        /// </summary>
        protected abstract TConnection CreateConnection(string connectionStringOrPath);

        /// <summary>
        /// Метка повтора попытки
        /// </summary>
        ...
        internal User Authorization(EnvironmentService envService, bool isExtraNet)
        {
        ...
        {
            _logger.Info($"БД {DbProviderName}: Получить SubDepartment пользователя");
        ...
        {
            _logger.Info($"БД {DbProviderName}: Получить описание окна загрузки");

            IEnumerable<LoaderDescription> loaderDescriptions;
            SubDepartment subDepartment = GetSubDepartmentForCurrentUser(currentUser);

            if (subDepartment != null && subDepartment.Code.ToLower().Contains("bim"))
            {
                loaderDescriptions = ExecuteQuery<LoaderDescription>(
                    $"SELECT * FROM {MainDB_Tables.LoaderDescriptions};");
            }
            else
            {
                loaderDescriptions = ExecuteQuery<LoaderDescription>(
                    $"SELECT * FROM {MainDB_Tables.LoaderDescriptions} " +
                    $"WHERE ({nameof(LoaderDescription.SubDepartmentId)} = 1 " +
                    $"OR {nameof(LoaderDescription.SubDepartmentId)} = @{nameof(User.SubDepartmentId)});",
                    new { currentUser.SubDepartmentId });
            }

            var descriptionsList = loaderDescriptions.ToList();
            if (descriptionsList.Count == 0)
                return null;

            Random rand = new Random();
            int index = rand.Next(descriptionsList.Count);
        ...
        {
            _logger.Info($"БД {DbProviderName}: Обновить данные по последнему запуску");
        ...
        {
            _logger.Info($"БД {DbProviderName}: Записать значение уровня одобрения от пользователя");
        ...
        internal void SetRevitUserName(string userName, User currentUser)
        {
            if (currentUser.RevitUserName == null || !currentUser.RevitUserName.Equals(userName))
            {
                try
                {
                    _logger.Info($"БД {DbProviderName}: Обновление имени Revit-пользователя с {currentUser.RevitUserName} на {userName}");
                ...
                {
                    _logger.Error(
                        $"БД {DbProviderName}: Не удалось обновить имени Revit-пользователя. Ошибка: {ex.Message}");
                }
            }
        }
        #endregion
